<?php
/**
 * Common bootstrap/helpers for AI endpoints.
 *
 * - Auth: includes/api_auth.php (JSON-only responses)
 * - DB: includes/db.php
 *
 * IMPORTANT:
 * - AI endpoints are tenant-scoped; always enforce tenant isolation and RBAC server-side.
 */
 ...
 */
function cnx_ai_require_csrf_for_write(): void {
...
/**
 * Resolve target tenant id for tenant-scoped operations.
 * - Prefer explicit ?tenant_id=
 * - else CompanyFilter (company_filter_id)
 * - else user's primary tenant
 */
function cnx_ai_resolve_target_tenant_id(array $userInfo): int {
 ...
 * Mirrors compliance rules.
 */
function cnx_ai_user_has_access_to_tenant(Database $db, array $userInfo, int $tenantId): bool {
...
/**
 * Schema-drift safe table availability check.
 *
 * @param string[] $requiredTables
 * @param ?string $migrationHint
 * @return array{ok:bool,missing?:string[],migration?:string}
 */
function cnx_ai_check_tables(Database $db, array $requiredTables, ?string $migrationHint = null): array {
    $missing = [];
    ...
    if (!empty($missing)) {
        ...
        if ($migrationHint) $out['migration'] = $migrationHint;
        return $out;
    }
    return ['ok' => true];
}
