-- Module: CollaboraNexio Phase 5 - External Collaboration
-- Version: 2025-01-23
-- Author: Database Architect
-- Description: Complete database schema for external collaboration features including secure sharing, version control, comments, and approval workflows
...
INSERT INTO users (id, tenant_id, username, email, full_name) VALUES
    (1, 1, 'john.doe', 'john.doe@demo.com', 'John Doe'),
    (2, 1, 'jane.smith', 'jane.smith@demo.com', 'Jane Smith'),
    (3, 1, 'bob.manager', 'bob.manager@demo.com', 'Bob Manager'),
    (4, 2, 'alice.cooper', 'alice.cooper@demo.com', 'Alice Cooper')
ON DUPLICATE KEY UPDATE email=VALUES(email);

-- Sample files (if not exists)
INSERT INTO files (id, tenant_id, file_name, file_size, mime_type, storage_path, uploaded_by) VALUES
    (1, 1, 'project_proposal.pdf', 2048576, 'application/pdf', '/storage/1/docs/proposal.pdf', 1),
    (2, 1, 'design_mockup.png', 1536000, 'image/png', '/storage/1/images/mockup.png', 2),
    (3, 1, 'quarterly_report.xlsx', 512000, 'application/vnd.ms-excel', '/storage/1/docs/report.xlsx', 1),
    (4, 2, 'contract_draft.docx', 768000, 'application/msword', '/storage/2/docs/contract.docx', 4)
...
INSERT INTO share_links (tenant_id, file_id, unique_token, created_by, expiration_date, max_downloads, title, description) VALUES
    (1, 1, 'SHR_' || MD5(CONCAT('token1', NOW())), 1, DATE_ADD(NOW(), INTERVAL 7 DAY), 10, 'Project Proposal Review', 'Please review and provide feedback'),
    (1, 2, 'SHR_' || MD5(CONCAT('token2', NOW())), 2, DATE_ADD(NOW(), INTERVAL 30 DAY), NULL, 'Design Mockup Share', 'Latest design iteration for client'),
    (1, 3, 'SHR_' || MD5(CONCAT('token3', NOW())), 1, NULL, 5, 'Q4 Report', 'Quarterly financial report');
...
-- Check active shares
SELECT 'Active share links' as info,
       sl.unique_token,
       f.file_name,
