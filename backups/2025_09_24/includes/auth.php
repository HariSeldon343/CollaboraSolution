...
class Auth {
    ...
    public function login(string $email, string $password): array {
        try {
            ...
            $stmt = $this->pdo->prepare("
                SELECT u.*, t.id as tenant_id, t.name as tenant_name, t.is_active as tenant_status
                FROM users u
    ...
    public function getCurrentUser(): ?array {
        ...
        try {
            $stmt = $this->pdo->prepare("
                SELECT u.id, u.name, u.email, u.role, u.avatar, u.tenant_id,
    ...
    public function switchTenant(int $tenantId): array {
        ...
        try {
            ...
            $stmt = $this->pdo->prepare("
                SELECT uta.*, t.name as tenant_name, t.is_active as tenant_status
            ...
            $_SESSION['user_role'] = $access['role'] ?? $_SESSION['user_role'];
    ...
    private function getUserTenants(int $userId): array {
        try {
            $stmt = $this->pdo->prepare("
                ...
                WHERE t.is_active = 1
                AND (u.id IS NOT NULL OR (uta.user_id IS NOT NULL AND uta.is_active = 1))
                ORDER BY t.name
            ");
