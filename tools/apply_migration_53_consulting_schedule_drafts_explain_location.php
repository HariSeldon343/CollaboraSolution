<?php
/**
 * One-click migration: Consulting schedule drafts - location + explain metadata (Migration 53).
 *
 * Adds optional columns to consulting_plan_schedule_drafts:
 * - location_city (VARCHAR): city for onsite/travel clustering
 * - explain_json (TEXT): JSON metadata about slot placement decisions
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/53_consulting_schedule_drafts_explain_location.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 53 applied (consulting schedule drafts explain/location)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 53: " . $e->getMessage() . "\n";
    exit(1);
}
