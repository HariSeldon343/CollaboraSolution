<?php
/**
 * One-click migration: Consulting plan items phases metadata (Migration 62).
 *
 * Adds optional columns to consulting_plan_items:
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/62_consulting_plan_items_phases.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 62 applied (consulting_plan_items phases metadata)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 62: " . $e->getMessage() . "\n";
    exit(1);
}
