<?php
/**
 * One-click migration: Consulting plan items locations (Migration 61).
 *
 * Adds optional column to consulting_plan_items:
 * - location_tenant_location_id (INT UNSIGNED NULL)
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/61_consulting_plan_items_locations.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 61 applied (consulting_plan_items.location_tenant_location_id)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 61: " . $e->getMessage() . "\n";
    exit(1);
}
