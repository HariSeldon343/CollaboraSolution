<?php
/**
 * One-click migration: Planning Estimate Wizard sessions (Migration 57).
 *
 * Creates consulting_plan_estimates table used to store server-side estimate drafts/snapshots.
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/57_consulting_plan_estimates.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 57 applied (consulting_plan_estimates)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 57: " . $e->getMessage() . "\n";
    exit(1);
}
