<?php
/**
 * One-click migration: Consulting (tenant 28) Service Catalog extensions (Migration 50).
 *
 * Adds optional columns:
 * - consulting_activity_types: service_code, category, aliases_json, base_days_min/max, complexity_score, default_phases_json, default_deliverables_json
 * - consulting_plans: estimate_json
 * - consulting_plan_items: assignee_user_id
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/50_consulting_service_catalog_extensions.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 50 applied (consulting service catalog extensions)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 50: " . $e->getMessage() . "\n";
    exit(1);
}
