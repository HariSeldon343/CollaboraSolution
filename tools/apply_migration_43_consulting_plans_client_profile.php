<?php
/**
 * One-click migration: Consulting Plans client profile fields (Migration 43).
 *
 * Adds optional columns on consulting_plans:
 * - client_business_description, client_desired_scope_hint, client_employee_count, client_sites_count, blueprint_standards_json
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/43_consulting_plans_client_profile.sql';
    if (!is_file($sqlFile)) {
    ...
    // Simple split by semicolon (same as other tools in repo)
    ...
    echo "OK: migration 43 applied (consulting_plans client profile fields)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 43: " . $e->getMessage() . "\n";
    exit(1);
}
