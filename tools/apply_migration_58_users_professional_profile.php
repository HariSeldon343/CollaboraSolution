<?php
/**
 * One-click migration: Users - professional profile fields (Migration 58).
 *
 * Adds optional columns to users:
 * - job_title (VARCHAR)
 * - skills_text (TEXT)
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/58_users_professional_profile.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 58 applied (users professional profile)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 58: " . $e->getMessage() . "\n";
    exit(1);
}
