<?php
/**
 * One-click migration: Consulting (tenant 28) Atomic services support (Migration 51).
 *
 * Adds optional columns to consulting_activity_types:
 * - standard_codes_json (TEXT): JSON array of standard codes covered by a service
 * - is_legacy_composite (TINYINT): marks deprecated combo services (kept for historical plans)
 *
...
try {
    ...
    $sqlFile = __DIR__ . '/../database/migrations/51_consulting_activity_catalog_atomic_services.sql';
    if (!is_file($sqlFile)) {
    ...
    echo "OK: migration 51 applied (consulting activity catalog atomic services)\n";
    exit(0);
} catch (Throwable $e) {
    http_response_code(500);
    echo "ERROR applying migration 51: " . $e->getMessage() . "\n";
    exit(1);
}
