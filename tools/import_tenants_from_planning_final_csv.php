<?php
/**
 * Tool: Import massivo Aziende (Tenants) da "Aziende_Planning_FINALE.CSV"
 *
 * - Dry-run di default (non scrive DB)
 * - Apply solo con ?apply=1 (web) o --apply (CLI)
 * - CSV: delimitatore ';' con header:
 *   Nome Originale;Ragione Sociale;Partita IVA;Sede Legale;Settore
 * - Idempotente: salta duplicati per Partita IVA (anche se già presenti)
 * - Crea: tenants + (tenant_locations se presente) + cartella root files
 *
 * CLI:
 *   php tools/import_tenants_from_planning_final_csv.php --file "C:\Users\...\Aziende_Planning_FINALE.CSV"
 *   php tools/import_tenants_from_planning_final_csv.php --file "..." --apply --user-id 19
 *
 * Web (super_admin):
 *   /CollaboraNexio/tools/import_tenants_from_planning_final_csv.php?apply=1
 */
...
function cnx_is_valid_utf8(string $s): bool {
    return preg_match('//u', $s) === 1;
}

function cnx_to_utf8(string $s): string {
    // Best-effort: if string is not valid UTF-8, try Windows-1252 -> UTF-8.
...
/**
 * Parse "Via X 10, 90100 Comune (PR)" or "Contrada Y, 90100 Comune (PR)".
 * If civico missing, default to "snc".
 */
function cnx_parse_sede_legale_planning(string $full): array {
    ...
    $last = cnx_norm_str(array_pop($parts)); // "CAP Comune (PR)"
    $addrLine = cnx_norm_str(implode(',', $parts)); // keep commas in street part
    ...
    // Try to split indirizzo vs civico from addrLine (last token that contains a digit or is SNC)
    ...
    if ($indirizzo === '') {
        // If we failed to split, fallback to full addrLine as indirizzo and civico=snc
        $indirizzo = $addrLine;
...
function cnx_map_settore(string $raw): string {
    $s = strtoupper(cnx_norm_str($raw));
    if ($s === '') return 'altro';

    if (strpos($s, 'SANIT') !== false || strpos($s, 'SOCIO-SANIT') !== false || strpos($s, 'OSPED') !== false || strpos($s, 'POLICLIN') !== false || strpos($s, 'LABORATOR') !== false) {
        return 'sanita';
    }
    if (strpos($s, 'AGRO') !== false || strpos($s, 'ALIMENT') !== false) {
        return 'alimentare';
    }
    if (strpos($s, 'CONSULENZA') !== false) {
        return 'consulenza';
    }
    if (strpos($s, 'IT') !== false || strpos($s, 'INFORMAT') !== false || strpos($s, 'TECNO') !== false) {
        return 'informatica';
    }
    if (strpos($s, 'VIGIL') !== false || strpos($s, 'MULTISERV') !== false || strpos($s, 'SERVIZ') !== false) {
        return 'servizi';
    }
    if (strpos($s, 'INGEGNER') !== false || strpos($s, 'PROGETTAZ') !== false || strpos($s, 'MANUTEN') !== false) {
        return 'consulenza';
    }
    if (strpos($s, 'COMMERC') !== false) {
        return 'commercio';
    }
    return 'altro';
}
...
function cnx_write_reports(array $report, string $baseName): array {
    ...
    $fp = @fopen($csvPath, 'wb');
    if ($fp) {
        fputcsv($fp, ['row', 'ragione_sociale', 'partita_iva', 'settore_raw', 'settore_mapped', 'result', 'tenant_id', 'message']);
...
if (is_cli()) {
    ...
    $_SERVER['REQUEST_URI'] = $_SERVER['REQUEST_URI'] ?? '/CollaboraNexio/tools/import_tenants_from_planning_final_csv.php';
}
...
if (!$filePath) {
    $filePath = 'c:\\Users\\aoedo\\Desktop\\Aziende_Planning_FINALE.CSV';
}
...
$tenantsHasCode = in_array('code', $tenantsCols, true);
...
if (!file_exists($filePath)) {
    ...
    echo '<pre>' . h(json_encode($report, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) . '</pre>';
    exit;
}
...
if (!$fp) {
    ...
    $required = ['RAGIONE SOCIALE', 'PARTITA IVA', 'SEDE LEGALE', 'SETTORE'];
    ...
    if (!$report['failed']) {
        ...
        while (($row = fgetcsv($fp, 0, ';', '"')) !== false) {
            ...
            $nomeOriginale = isset($idx['NOME ORIGINALE']) ? cnx_norm_str((string)($row[$idx['NOME ORIGINALE']] ?? '')) : '';
            $ragione = cnx_norm_str((string)($row[$idx['RAGIONE SOCIALE']] ?? ''));
            ...
            $settoreMapped = cnx_map_settore($settoreRaw);
            ...
            // Nota: alcuni dataset possono contenere CF/identificativi non validi come P.IVA (checksum).
            // Non blocchiamo per checksum: si usa check length only.
            ...
            $parsed = cnx_parse_sede_legale_planning($sedeRaw);
            if (!($parsed['ok'] ?? false)) {
                $report['skipped_invalid']++;
                $report['rows'][] = [
                    'row' => $rowNum,
            ...
            try {
                ...
                if ($tenantsHasCode && $nomeOriginale !== '') {
                    $tenantData['code'] = $nomeOriginale;
                }
...
$baseName = 'tenants_import_planning_final_' . date('Ymd_His');
...
echo '<h2>Import Tenants — Planning Finale</h2>';
