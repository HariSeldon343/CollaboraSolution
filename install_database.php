<?php
/**
 * Database Installation Script for CollaboraNexio
 * Installs Phase 2 and Phase 3 schemas
 */
...
echo "<h1>CollaboraNexio Database Installation</h1>";
echo "<pre>";
...
try {
    ...
    echo "Installing Phase 2: Document Management System...\n";

    // Read and execute Phase 2 SQL
    $phase2_sql = file_get_contents(__DIR__ . '/install_phase2_fixed.sql');

    // Split by semicolons but ignore those inside quotes
    $statements = preg_split('/;\s*$/m', $phase2_sql);
    ...
    foreach ($statements as $statement) {
        ...
        if (!empty($statement) && stripos($statement, 'DELIMITER') === false) {
            try {
                ...
                // Ignore duplicate key errors for demo data
                if ($e->getCode() != '23000') {
    ...
    // Now install Phase 3
    echo "Creating Phase 3 fixed SQL file...\n";

    // Create Phase 3 fixed SQL
    $phase3_fixed = <<<'SQL'
...
SQL;
    ...
    file_put_contents(__DIR__ . '/install_phase3_fixed.sql', $phase3_fixed);

    echo "Installing Phase 3: Productivity Tools...\n";

    // Read and execute Phase 3 SQL
    $phase3_sql = file_get_contents(__DIR__ . '/install_phase3_fixed.sql');
    $statements = preg_split('/;\s*$/m', $phase3_sql);
    ...
    echo "\n✅ Phase 3 installed successfully!\n\n";
    ...
    $tables = [
        'Phase 2' => ['folders', 'files', 'file_permissions', 'file_shares', 'file_activity_logs'],
        'Phase 3' => ['calendars', 'events', 'task_lists', 'tasks', 'time_entries']
    ];

    foreach ($tables as $phase => $table_list) {
        echo "\n$phase Tables:\n";
    ...
    echo "- Document Management System (folders, files, sharing)\n";
    echo "- Calendar System (events, recurring events, reminders)\n";
    echo "- Task Management (kanban boards, tasks, time tracking)\n";
